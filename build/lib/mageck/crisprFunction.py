...
def calculate_gene_lfc(args,lfcval,sort_id,n_lower,sgrna2genelist,destkeys,validsgrna=None,ispos=False):
  ...
  for i in sort_id:
    ...
    if args.gene_lfc_method=='alphamean' or args.gene_lfc_method=='alphamedian':
      ...
      genesglfc[targetgene]+=[lfcval[i]]
  genelfc={}
  for (gid,vl) in list(genesglfc.items()):
...
def crispr_test(tab,ctrlg,testg,varg, destfile,sgrna2genelist,args):
  ...
  tabctrl={k:[v[i] for i in range(len(v)) if i in ctrlg] for (k,v) in list(tab.items())}
  tabtest={k:[v[i] for i in range(len(v)) if i in testg] for (k,v) in list(tab.items())}
  ...
  if len(varg)>1:
    tabctrlmod={k:[v[i] for i in range(len(v)) if i in varg] for (k,v) in list(tab.items())}
  #elif len(ctrlg)>1 and args.variance_from_all_samples==False: # more than 1 controls
  elif len(ctrlg)>1: # more than 1 controls
    tabctrlmod={k:[v[i] for i in range(len(v)) if i in ctrlg] for (k,v) in list(tab.items())}
  else: # only 1 control: use all the samples for estimation
    tabctrlmod={k:[v[i] for i in range(len(v)) if i in (ctrlg+testg)] for (k,v) in list(tab.items())}
  # treatment matrix
  tabtreatmod={k:[v[i] for i in range(len(v)) if i in (testg)] for (k,v) in list(tab.items())}
  ...
  if False:
    ...
    t_var_t=getVars(list(tabtreatmod.values()))
    n_ctrl=len(ctrlg)
    n_test=len(testg)
...
def magecktest_parsetreatmentsfromday0(args,samplelabelindex):
  ...
  """
  samples=[s for s in list(samplelabelindex.keys())]
...
def magecktest_main(args):
  ...
  for cpindex in range(len(supergroup_treat)):
    ...
    #
    # normalization
    cttab_sel={k:([v[i] for i in selected_samples_id]) for (k,v) in list(cttab.items())}; # controlgroup do not overlap with treatgroup
    norm_msg=NormMsgs()
