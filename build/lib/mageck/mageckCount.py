...
def mageckcount_printdict(dict0,args,ofile,ounmappedfile,sgdict,datastat,sep='\t'):
  ...
  if args.fastq is not None:
    ...
    for (label,dsitem) in list(datastat.items()):
...
def mageck_printdict(dict0,args,sgdict,sampledict,sampleids):
  ...
  for (k,v) in list(sampledict.items()):
    mapres_list[v]=k
  if len(sampledict)>0:
...
def mageckcount_processfastq(args,genedict,sgdict):
  ...
  if len(sgdict)>0:
    allmappeddict={k:v for (k,v) in list(alldict.items()) if k in sgdict} # only keep those with known sgRNAs
  else:
...
def mageckcount_processcounttable(args,genedict,sgdict):
  ...
  datastat={}
  for (sampleid, sampleindex) in list(sampleids.items()):
    ...
    ntotalsgcount=0
    nrdcnt=[]
    for (sgid, cntv) in list(gtab.items()):
...
def mageckcount_main(args):
  ...
  if hasattr(args,'count_table') and args.count_table != None:
    ...
    sgdict={k:(k,v) for (k,v) in list(mapptab.items())}
  else:
