...
class VisualRValue:
  ...
  def loadGeneExp(self,genelist,nttab,sgrna2genelist,collabels):
    ...
    for gene in genelist:
      sglist=[ k for (k,v) in list(sgrna2genelist.items()) if v==gene]
      ntgene={k:v for (k,v) in list(nttab.items()) if k in sglist}
      npl+=1
      # load to file
      valstring='list('
      vstrlist=[]
      for (k,v) in list(ntgene.items()):
...
def plot_main(args):
  ...
  # normalization
  cttab_sel={k:([v[i] for i in treatgroup]) for (k,v) in list(cttab.items())}; # controlgroup do not overlap with treatgroup
