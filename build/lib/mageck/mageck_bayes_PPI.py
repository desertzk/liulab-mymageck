...
def PPI_weighting_rewiring(self):
    weighting_record=[]
    for gene, neighboring_gene_weighting in list(self.PPI_data.items()):
    ...
    for gene, neighboring_gene_weighting in list(self.PPI_data.items()):
        for neighboring_gene, weighting in list(neighboring_gene_weighting.items()):
            neighboring_gene_weighting[neighboring_gene]=weighting/weighting_correction_ratio

def PPI_coverge_diagnosis(self):
    PPI_genes=list(self.PPI_data.keys())
    crispr_genes=list(self.allgenedict.keys())
...
def PPI_network_plotting(self,plotting=False,self_adjust=False,constant=0):
    ...
    outliers_record=[]
    for gene,beta_value in list(beta1_estimate.items()):
        ...
        if Counter([math.isnan(i) for i in beta_value])[True]==0:
            ...
            for neighboring_gene, weighting in list(self.PPI_data[gene.upper()].items()):
