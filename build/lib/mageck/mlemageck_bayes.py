...
def beta_non_PPI_prior_calculation(allgenedict):
    ...
    '''
    temp_beta=[(sk.nb_count.shape[1],sk.beta_estimate) for gene,sk in list(allgenedict.items())]
...
def dispersion_reestimate_sup(input_variable):
    ...
    if len(tgid_list)>0:
        ...
        temp={tgid:tginst for tgid,tginst in list(self.allgenedict.items()) if tgid in tgid_list}
