...
def bayes_selection_constnat_optimization(self):
    ...
    sgRNA_number=[[sgRNA_number,count] for sgRNA_number,count in list(Counter(sgRNA_number).items())]
    ...
    selection_percentage=[[constant,abs(percentage-removal_ratio)] for constant,percentage in list(constant_percentage.items())]
...
def background(self):
    ...
    for (tgid,tginst) in list(data.allgenedict.items()):
